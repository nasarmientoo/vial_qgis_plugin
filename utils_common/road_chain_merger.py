...
def _ensure_integer_id_field(src_lines, id_name="id"):
    """
    Garantiza que exista un campo entero 'id'. Si no existe, lo crea con
    el valor de to_int($id). Devuelve una capa temporal con el campo presente.
    """
    if src_lines.fields().indexOf(id_name) != -1:
        return _copy_layer_temp(src_lines, "lines_with_id")
